...
//
// ### function listObjects (container, options, callback)
// #### @container {string} Name of the container to get object list for
// #### @params {object}
// #### @callback {function} Continuation to respond when complete
// Gets the list of objects in specified `container` filters by supplied params
// Note: only filter by prefix is implemented currently
//
Cloudfiles.prototype.listObjects = function (container, params, callback) {
  var object_list = []; // array to contain objects found

  var self = this, 
      containerName = container instanceof cloudfiles.Container ? container.name : container;

  // ensure we return results as json
  params.format = 'json';

  // load the query string helper module in as it makes life easier
  var querystring = require('querystring');

  // construct the options to send to Rackspace
  options = {
    method: 'GET', 
    client: self,
    uri: self.storageUrl(containerName)+'/?'+querystring.stringify(params)
  };
  ...
  common.rackspace(
    ...
    function(body, res) {
      if(body) {
	object_list = JSON.parse(body);
      }
      callback(null, object_list);
    });
};
